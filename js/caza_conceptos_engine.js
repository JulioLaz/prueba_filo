...
// // === Conteo global de la actividad (todas las frases de todos los párrafos) ===
// const TOTAL_CONCEPTS = CFG.levels.reduce((acc, lv) => acc + ((lv.concepts || []).length), 0);

// overallFoundCount = 0;
// globalFound = new Set();

// // Inicializa el progreso para el hub (arranca en 0 encontrados)
...
// function hubSaveConcept(partial) {
//   const key = `tema.${hubTemaSlug()}.caza`; // 👈 clave correcta
//   let prev = {};
//   try { prev = JSON.parse(sessionStorage.getItem(key)) || {}; } catch {}

//   const state = {
//     total: partial.total ?? prev.total ?? 0,
//     found: partial.found ?? prev.found ?? 0,
