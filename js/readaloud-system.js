...
/* readaloud-system.js - Sistema Modular de Lectura en Voz Alta
 * Versión: 1.1 - CÁLCULO DE SECCIONES CORREGIDO
 * Uso: Incluir en cualquier content.html con configuración específica
 */
...
(function() {
    ...
    console.log('🎤 === INICIANDO READALOUD SYSTEM v1.1 ===');
    ...
    class ReadAloudSystemModular {
        constructor() {
            ...
            console.log(`[ReadAloud] Iniciando sistema para tema: ${this.config.tema}`);
            console.log(`[ReadAloud] Threshold configurado: ${this.config.threshold * 100}%`);
            console.log(`[ReadAloud] Secciones configuradas: ${this.config.sections}`);
        ...
        initializeSystem() {
            // ⭐ CORREGIDO: Usar total de secciones de la configuración como FUENTE DE VERDAD
            this.sections = document.querySelectorAll('.study-section');
            this.totalSections = this.config.sections; // FORZAR el valor correcto
            this.currentSectionIndex = 0;
            this.completedSections = new Set();
            this.readAloudSystems = new Map();
            
            console.log(`[ReadAloud] 🔍 DIAGNÓSTICO DE SECCIONES:`);
            console.log(`   Secciones en DOM: ${this.sections.length}`);
            console.log(`   Secciones configuradas: ${this.totalSections}`);
            console.log(`   Usando como total: ${this.totalSections} (de configuración)`);
            ...
            if (this.sections.length !== this.totalSections) {
                console.warn(`[ReadAloud] ⚠️ INCONSISTENCIA: DOM=${this.sections.length}, Config=${this.totalSections}`);
                console.warn(`[ReadAloud] Se usará el valor de configuración: ${this.totalSections}`);
            }
        ...
        // ⭐ CORREGIDO: Usar totalSections de configuración
        updateGlobalProgress() {
            const progressFill = document.getElementById('progress-fill');
            const progressCounter = document.getElementById('progress-counter');
            
            if (progressFill && progressCounter) {
                // ⭐ USAR this.totalSections (de configuración) en lugar de this.sections.length
                const progress = (this.completedSections.size / this.totalSections) * 100;
                const percentage = Math.round(progress);
                
                console.log(`[ReadAloud] 📊 PROGRESO: ${this.completedSections.size}/${this.totalSections} = ${percentage}%`);
                
                progressFill.style.width = `${progress}%`;
                progressCounter.textContent = `${this.completedSections.size} / ${this.totalSections} secciones`;
            }
        }
    ...
    // Auto-inicializar si existe configuración global
    if (window.ReadAloudSystemConfig) {
        const instance = new ReadAloudSystemModular();
        
        // ⭐ EXPONER INSTANCIA GLOBALMENTE para debugging
        window.readAloudSystemInstance = instance;
        
        console.log('🎤 ReadAloud System inicializado y expuesto globalmente');
        console.log('🔍 Para debugging usa: readAloudSystemInstance');
    }
